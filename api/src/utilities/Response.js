...
class Response {
	...
	static expressValidationFail(res, message, code = HttpStatus.StatusCodes.NOT_FOUND, resCode = HttpStatus.StatusCodes.NOT_FOUND, extra = {}, validationErrorList = []) {
		const resObj = { success: false };
		if (_.isObjectLike(message)) {
			resObj.message = message.validationErrorList[0].msg;
			resObj.data = message.validationErrorList;
			resObj.code = message.code || HttpStatus.StatusCodes.NOT_FOUND;
			resObj.resCode = message.resCode || resObj.code;
			if (!_.isEmpty(message.extra) && _.isObjectLike(message.extra)) {
				resObj.extra = message.extra;
